...
- 报表支持 Markdown、HTML、PDF 和 Excel 导出，并可选择使用 OpenAI 兼容 API 润色。
- Excel 支持内置及自定义表格模板，可自由配置最多 30 个表头；系统按表头语义自动匹配任务事实，大模型模式还会润色叙述性字段。
...
报表页默认提供“标准工作汇报”和“任务跟进清单”两套模板。点击模板旁的“编辑”可修改当前模板，也可新建或删除模板。表头支持每行一个或使用逗号分隔，最多 30 列。

未启用大模型时，任务名称、状态、进度、需求人、负责人、本期完成、下一步、对接人、风险、记录时间和截止日期等常见表头由本地规则匹配。启用大模型并生成汇报后，导出 Excel 会再次调用模型，根据自定义表头语义逐行映射并润色。模型只能使用任务和进度记录中的事实，无法匹配的字段留空。

Excel 工作簿包含标题、统计周期、模板名称、筛选表头和任务明细。日期、整数和百分比使用真实 Excel 数据类型，表头冻结并根据内容设置列宽，方便后续筛选、排序和二次编辑。
